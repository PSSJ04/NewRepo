...
function App() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get('https://newrepo-production-7231.up.railway.app/api/items');
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://newrepo-production-7231.up.railway.app/api/items', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://newrepo-production-7231.up.railway.app/api/items/${id}`);
      fetchItems();
    }catch (err) {
